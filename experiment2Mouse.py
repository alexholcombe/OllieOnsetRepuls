...
for circlePosition in range(positionMin, positionMax, 5):
        for direction in [-1,1]:
            for axis in ['x', 'y']:
                stimList.append({'circlePosition': circlePosition, 'direction':direction, 'axis': axis})
...
def collectResponse(probe, quitExperiment):
    respFinished = False
    frameCounter = 0
    
    #setting mouse and probe position
    ...
    while not quitExperiment and not respFinished:
        ...
        probe.pos = mousePos  # Update the probe's position to follow the mouse
        ...
        buttons = mouse.getPressed()  # Returns a tuple (left, middle, right)
        if buttons[0]:  # Left mouse button clicked
            respFinished = True
    
    respTime = rtClock.getTime()
    return quitExperiment, respTime, probe.pos
...
for thisTrial in trials:  # handler can act like a for loop
    ...
    for frame in range(stimDurFrames):
        ...
        quitExperiment, respTime, probePos = collectResponse(probe, quitExperiment)
        ...
        trials.addData('probePosX', probePos[0])  # Log the x-coordinate of the probe
        trials.addData('probePosY', probePos[1])
